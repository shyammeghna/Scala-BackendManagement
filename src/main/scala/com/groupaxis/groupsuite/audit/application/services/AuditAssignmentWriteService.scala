...
object AuditAssignmentWriteService {

  final val Name = "audit-assignment-write-service"

  def props(diffRepo: JdbcDifferenceWriteRepository): Props = Props(classOf[AuditAssignmentWriteService], diffRepo)
}

class AuditAssignmentWriteService(diffRepo: JdbcDifferenceWriteRepository) extends Actor with Logging {
  ...
  override val supervisorStrategy =
    OneForOneStrategy(maxNrOfRetries = 10, withinTimeRange = 1.minute) {
      ...
      case e: Exception =>
        logger.warn("[AuditAssignmentWriteService] Exception has been received, so restarting the actor " + e.getMessage)
        e.printStackTrace()
        Restart
    }
  ...
  def receive: Receive = {
    ...
    case CreateBackendAssignment(username, date, newEntity) =>
      logger.info(s" createBackendAssignment received $CreateBackendAssignment")
      sender() ! RoutingCreationDone()
    case CreateDistributionAssignment(username, date, newEntity) =>
      logger.info(s" CreateDistributionAssignment received $CreateDistributionAssignment")
      sender() ! RoutingCreationDone()
    case CreateFeedbackAssignment(username, date, newEntity) =>
      logger.info(s" CreateFeedbackAssignment received $CreateFeedbackAssignment")
      sender() ! RoutingCreationDone()
    case CreateImport(username, date, newFileName, status) =>
      logger.info(s" CreateImport received $CreateImport")
      sender() ! RoutingCreationDone()
    case CreateImportBackup(username, date, newFileName, status) =>
      logger.info(s" CreateImportBackup received $CreateImportBackup")
      sender() ! RoutingCreationDone()
    case CreateExport(username, date, newFileName, status) =>
      logger.info(s" CreateExport received $CreateExport")
      sender() ! RoutingCreationDone()
    case CreateRuleOverviewCSV(username, date, newFileName) =>
      logger.info(s" CreateRuleOverviewCSV received $CreateRuleOverviewCSV")
      sender() ! RoutingCreationDone()
    case CreateAssignmentOverviewCSV(username, date, newFileName) =>
      logger.info(s" CreateAssignmentOverviewCSV received $CreateAssignmentOverviewCSV")
      sender() ! RoutingCreationDone()
    case UpdateBackendAssignment(username, date, original, update) =>
      logger.info(s" UpdateBackendAssignment received $UpdateBackendAssignment")
      sender() ! RoutingCreationDone()
    case UpdateDistributionAssignment(username, date, original, update) =>
      logger.info(s" UpdateDistributionAssignment received $UpdateDistributionAssignment")
      sender() ! RoutingCreationDone()
    case UpdateFeedbackAssignment(username, date, original, update) =>
      logger.info(s" UpdateFeedbackAssignment received $UpdateFeedbackAssignment")
      sender() ! RoutingCreationDone()
    case CreateRule(username, date, newEntity) =>
      logger.info(s" CreateRule received $CreateRule")
      sender() ! RoutingCreationDone()
    case UpdateRule(username, date, original, update) =>
      logger.info(s" UpdateRule received $UpdateRule")
      sender() ! RoutingCreationDone()
    case DeleteRule(username, date, deleteEntity) =>
      logger.info(s" DeleteRule received $DeleteRule")
      sender() ! RoutingCreationDone()
  }
}